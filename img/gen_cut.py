...
s.append(f'<rect x="-6" y="{ECR1}" width="{L+22}" height="{ISO1-ECR1}" fill="#F26205" fill-opacity=".13"/>')
s.append('<g class="cch" style="--d:1">')
s.append('<g clip-path="url(#clipIso)" stroke="#FBC49B" stroke-opacity=".6" stroke-width="2">')
hx = -40
while hx < L + 60:
...
marques = []
s.append('<g class="rappels cch" style="--d:8" stroke="#F26205" stroke-opacity=".65" stroke-width="1.6" stroke-linecap="round">')
for nom, x, y, sens in reperes:
    ax, ay = page(x, y)
    mx, my = ax + sens * DIST * NX, ay + sens * DIST * NY
    s.append(f'<line x1="{f(ax)}" y1="{f(ay)}" x2="{f(mx)}" y2="{f(my)}"/>')
    s.append(f'<circle cx="{f(ax)}" cy="{f(ay)}" r="2.6" fill="#F26205" stroke="none"/>')
    marques.append((nom, mx, my))
# gouttière : rappel vers le haut-gauche
s.append(f'<line x1="{f(gx)}" y1="{f(gy + 12)}" x2="62" y2="398"/>')
s.append(f'<circle cx="{f(gx)}" cy="{f(gy + 12)}" r="2.6" fill="#F26205" stroke="none"/>')
